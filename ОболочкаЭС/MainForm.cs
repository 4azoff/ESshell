...
namespace ОболочкаЭС
{
    public partial class MainForm : Form
    {
        ...
        private void начатьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            else { MessageBox.Show("ЭС не открыта "); }
        ...
        private void btnAddDomainCtx_Click(object sender, EventArgs e)
        {
            btnAddDomain_Click(sender, e);
        }

        private void lbxRules_SelectedIndexChanged(object sender, EventArgs e)
        {
            int ind = lbxRules.SelectedIndex;
            lbxIF.Items.Clear();
            lbxThen.Items.Clear();
            if (lbxRules.SelectedIndex >= 0)
            {
                string rname = lbxRules.Items[ind].ToString().Split(':')[0].Trim();
                Rule ChRule = kbase.rules.Find(delegate (Rule r) { return r.name == rname; });
                tbRuleName.Text = ChRule.name;
                foreach(var f in ChRule.sendFact)
                {
                    lbxIF.Items.Add(f.varName + " = " + f.value);   
                }
                foreach (var f in ChRule.concFact)
                {
                    lbxThen.Items.Add(f.varName + " = " + f.value);
                }
            }
